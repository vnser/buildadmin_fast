    ...
    dragSort = () => {
        ...
        Sortable.create(el, {
            ...
            onMove: (evt, originalEvent) => {
                const draggedElement = evt.dragged.querySelector('.dataset');
                const relatedElement = evt.related.querySelector('.dataset');
                if (draggedElement){
                    const draggedId = draggedElement.getAttribute('data-id');
                    const draggedParentId = draggedElement.getAttribute('data-parent-id');

                    const relatedId = relatedElement.getAttribute('data-id');
                    const relatedParentId = relatedElement.getAttribute('data-parent-id');

                    // 判断是否同一层级（即 parentId 相同）
            ...
            onEnd: (evt: Sortable.SortableEvent) => {
                ...
                // const moveWeight = moveRow.weigh
                // const replaceWeight = replaceRow.weigh
                // console.log(moveRow, replaceRow)
                ...
                this.api.sortableApi(moveRow[this.table.pk!], replaceRow[this.table.pk!]).then((res)=>{
                    // this.table.data = []
                    // console.log(res)
                    // moveRow.weigh = replaceWeight
                    // replaceRow.weigh = moveWeight
                    this.onTableHeaderAction('refresh', {})
                }).finally((res) => {
