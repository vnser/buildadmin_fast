...
class ClickCaptcha
{
    ...
    public function __construct(array $config = [])
    {
        ...
        // 清理过期的验证码
        Db::name('_captcha')->where('expire_time', '<', time())->delete();
    }
    ...
    {
        $key     = md5($id);
        $captcha = Db::name('_captcha')->where('key', $key)->find();
        if ($captcha) {
            // 验证码过期
            if (time() > $captcha['expire_time']) {
                Db::name('_captcha')->where('key', $key)->delete();
                return false;
            }
            ...
            if ($unset) Db::name('_captcha')->where('key', $key)->delete();
            return true;
        } else {
            return false;
        }
