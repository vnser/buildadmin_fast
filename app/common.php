...
{
    $prefixes = ['User', 'Member', 'Pro'];
    $suffix = rand(1000, 9999); // 随机数字
    $prefix = $prefixes[array_rand($prefixes)]; // 随机前缀
    return $prefix . $suffix;
}

/**
 * 根据创建时间计算时间差，并返回类似“几秒前”、“几分钟前”、“几小时前”、“几天前”等自然语言的时间描述。
 * @param $timestamp
 * @return string
 */
function timeAgo($timestamp)
{
    ...
    $timeDiff = $currentTime - $timestamp; // 计算时间差（秒）

    if ($timeDiff < 60) {
        return $timeDiff . '秒前'; // 小于 60 秒
    } elseif ($timeDiff < 3600) {
        return floor($timeDiff / 60) . '分钟前'; // 小于 1 小时
    } elseif ($timeDiff < 86400) {
        return floor($timeDiff / 3600) . '小时前'; // 小于 1 天
    } elseif ($timeDiff < 604800) {
        return floor($timeDiff / 86400) . '天前'; // 小于 7 天
    } elseif ($timeDiff < 2592000) {
        return floor($timeDiff / 604800) . '周前'; // 小于 1 个月
    } elseif ($timeDiff < 31536000) {
        return floor($timeDiff / 2592000) . '个月前'; // 小于 1 年
    } else {
        return floor($timeDiff / 31536000) . '年前'; // 超过 1 年
    }
}